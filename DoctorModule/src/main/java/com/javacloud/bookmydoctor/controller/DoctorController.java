...
import com.javacloud.bookmydoctor.service.DoctorService;
...
public class DoctorController {
	...
	@PostMapping(path = "/add-doctor/{id}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public DoctorResponse addDoctor(@RequestBody Doctor bean, @PathVariable("id") int id) {
		DoctorResponse response = new DoctorResponse();
		if (service.addDoctor(bean, id)) {
	...
	@GetMapping(path = "/search-doctorCity/{city}", produces = MediaType.APPLICATION_JSON_VALUE)
	public DoctorResponse searchDoctor(@PathVariable("city") String city) {
		DoctorResponse response = new DoctorResponse();
		List<Doctor> bean = service.searchDoctor(city);
