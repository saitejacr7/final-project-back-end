...
public class DoctorController {
	...
	@GetMapping(path = "/template/search-doctorCity/{city}", produces = MediaType.APPLICATION_JSON_VALUE)
	public DoctorResponse searchDoctor(@PathVariable("city") String city) {

		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<DoctorResponse> entity = new HttpEntity<DoctorResponse>(headers);
		return restTemplate.exchange("http://localhost:8082/search-doctorCity/" + city, HttpMethod.GET, entity,
				DoctorResponse.class).getBody();
