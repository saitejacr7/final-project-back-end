...
public class PatientController {
	...
	@PostMapping(path = "/template/add-patient/{id}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public PatientResponse addPatient(@RequestBody Patient patient, @PathVariable("id") int id) {
		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<Patient> entity = new HttpEntity<Patient>(patient, headers);

		return restTemplate
				.exchange("http://localhost:8083/add-patient/" + id, HttpMethod.POST, entity, PatientResponse.class)
				.getBody();
	}
	...
	public PatientResponse viewAllPatients() {
		...
		HttpEntity<PatientResponse> entity = new HttpEntity<PatientResponse>(headers);
		return restTemplate
				.exchange("http://localhost:8083/view-allpatients", HttpMethod.GET, entity, PatientResponse.class)
				.getBody();
	}

	@DeleteMapping(path = "/template/delete-patient/{patientId}", produces = MediaType.APPLICATION_JSON_VALUE)
	public PatientResponse deletePatient(@PathVariable("patientId") int patientId) {
		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<PatientResponse> entity = new HttpEntity<PatientResponse>(headers);
		return restTemplate.exchange("http://localhost:8083/delete-patient/" + patientId, HttpMethod.DELETE, entity,
