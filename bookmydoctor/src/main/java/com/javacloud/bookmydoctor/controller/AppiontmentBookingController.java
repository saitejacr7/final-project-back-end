...
public class AppiontmentBookingController {
	...
	@GetMapping(path = "/template/get-appiontmentbyname/{doctorName}", produces = MediaType.APPLICATION_JSON_VALUE)
	public AppiontmentBookingResponse getAppiontmentByName(@PathVariable("doctorName") String doctorName) {
		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<String> entity = new HttpEntity<String>(headers);
		return restTemplate.exchange("http://localhost:8081/get-appiontmentbyname" + doctorName, HttpMethod.GET, entity,
				AppiontmentBookingResponse.class).getBody();
	}
