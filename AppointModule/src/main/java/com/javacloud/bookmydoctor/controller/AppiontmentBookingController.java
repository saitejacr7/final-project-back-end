...
import com.javacloud.bookmydoctor.service.AppiontmentBookingService;
...
public class AppiontmentBookingController {
	...
	@PostMapping(path = "/add-appiontment", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	@GetMapping(path = "/get-appiontmentbyname/{doctorName}", produces = MediaType.APPLICATION_JSON_VALUE)
	public AppiontmentBookingResponse getAppiontmentByName(@PathVariable("doctorName") String doctorName) {
		AppiontmentBookingResponse response = new AppiontmentBookingResponse();
		List<AppiontmentBooking> list = service.getAppiontmentsByName(doctorName);
		if (list != null) {
			response.setStatusCode(201);
			response.setMessage("Success");
			response.setDescription("Appiontment found");
